...
export async function GET(request: NextRequest) {
  try {
    ...
    return jsonError(error instanceof Error ? error.message : "Errore caricamento file", 500);
  }
}
...
export async function POST(request: NextRequest) {
  try {
    ...
    return jsonError(error instanceof Error ? error.message : "Errore upload file", 500);
  }
}
